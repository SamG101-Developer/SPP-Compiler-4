    ...
    def do_semantic_analysis(self, scope_handler: ScopeHandler, **kwargs) -> None:
        ...
        for branch in self.branches:
            ...
            if self.comp_operator and branch.comp_operator:
                ...
                exception.add_info(
                    pos=self.comp_operator.pos,
                    tag_message=f"Comparison operator '{self.comp_operator}' found here.")
                exception.add_error(
                    pos=branch.comp_operator.pos,
                    ...
                    tag_message=f"Subsequent comparison operator '{branch.comp_operator}' found here.",
                    tip=f"Remove the comparison operator 1 of the 2 places.")
                raise exception
        ...
        if kwargs.get("assignment", False):
            if Seq(self.branches).map(lambda b: b.body.members[-1].infer_type(scope_handler, **kwargs)).unique_items().length > 1:
                ...
                exception = SemanticError()
                exception.add_info(
                    pos=conflicting_types[0][1].pos,
                    tag_message=f"Type '{conflicting_types[0][0]}{conflicting_types[0][1]}' inferred here.")
                exception.add_error(
                    pos=conflicting_types[1][1].pos,
                    error_type=SemanticErrorType.TYPE_ERROR,
                    message=f"Conflicting types found in if-expression being used for assignment.",
                    tag_message=f"Type '{conflicting_types[1][0]}{conflicting_types[1][1]}' inferred here.",
            ...
            if not self.branches[-1].is_else_branch():
                ...
                exception.add_info(
                    pos=self.pos,
                    tag_message=f"If-expression declared here.")
                exception.add_error(
                    pos=self.branches[-1].pos,
                    error_type=SemanticErrorType.VALUE_ERROR,
                    message=f"Missing else branch in if-expression being used for assignment.",
                    tag_message=f"Last pattern block found here.",
                    tip=f"Add an else branch to the if-expression.")
                raise exception
